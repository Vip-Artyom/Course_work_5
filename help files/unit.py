...
class BaseUnit(ABC):
    ...
    def equip_weapon(self, weapon: Weapon):
        """Герой получает оружие"""
        self.weapon = weapon
        return f"{self.name} экипирован оружием {self.weapon.name}"

    def equip_armor(self, armor: Armor):
        """Герой получает броню"""
        self.armor = armor
        return f"{self.name} экипирован броней {self.armor.name}"

    def _count_damage(self, target: BaseUnit) -> int:
        """Функция расчета урона игрока, расчета брони цели и уменьшение выносливости атакующего при ударе"""
...
class EnemyUnit(BaseUnit):
    ...
    def hit(self, target: BaseUnit) -> str:
        ...
        if self.stamina * self.unit_class.skill.stamina < self.weapon.stamina_per_hit:
            return (
                f"{self.name} попытался использовать {self.weapon.name}, "
                f"но у него не хватило выносливости."
            )
